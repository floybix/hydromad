...
store $v_s$), and optionally \code{v\_3} and/or \code{v\_q}. See the
...
minimise a given objective function.
...
<<obj-fun>>=
body(hydromad.getOption("objective"))
@

The \code{fitStat} function implements a generalisation of the
familiar $R^2$ (coefficient of efficiency) statistic:

\begin{equation}
  \mathrm{fitStat}(Q,X) = \frac{ \sum |Q_* - X_*|^2 }{ \sum |Q_* - Z_*|^2 }
\end{equation}
