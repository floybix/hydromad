...
the cross-correlation changes over time. The \pkg{ihacres} package includes
...
An \code{ihacres} object encapsulates the chosen model functions, including
...
defaults are taken from \code{ihacres.options()}, and they can be
over-ridden by arguments to the \code{ihacres} function.

<<model, echo = TRUE>>=
cotterMod <- ihacres(ts90s, sma = "cwi", routing = "expuh")
print(cotterMod)
@
...
\code{ihacres.options(objective = )}.
...
store.\footnote{This default can be changed using
  \code{ihacres.options()} -- see the user manual.}
...
See the help pages \code{help("ihacres")} and
...
page of each function.\footnote{Note that to get help for generic
  functions it is necessary to specify the method for \code{ihacres}
  objects: e.g. \code{?predict.ihacres} or \code{?xyplot.ihacres}}

View the modelled and observed streamflow time series using
\code{xyplot}, as in Figure \ref{fig:obs-mod-plot}.
Figures \ref{fig:print-ihacres} and \ref{fig:summary-ihacres} show the
...
print(cotterFit)
@
\caption{\label{fig:print-ihacres}
...
summary(cotterFit)
@
\caption{\label{fig:summary-ihacres}
...
ihSpec <- ihacres(ts90s, sma = "cwi", routing = "uh", rfit = "sriv")
...
\code{ihacres.options(trace = TRUE)} and/or
\code{ihacres.options(catch.errors = FALSE)} to track down what is
happening.
...
%  \item using a different prefilter (for \code{tf.ls.fit} and \code{tf.sriv.fit});
  \item changing the optimisation method and/or settings: see \code{ihacres.options()}.
\end{itemize}
