...
\begin{abstract}
  ...
  analysis capabilities.  The main design goals were
  \emph{flexibility}, to explore research questions and develop
  improved methods, and \emph{simplicity} of the basic framework.  The
...
%A definitive version was subsequently published in PUBLICATION TITLE,
%[VOL#, ISSUE#], DATE, DOI.
...
at large scales \citep{WagenerEtAl:2009}. Given that the data
available in most catchments is badly limited in terms of duration,
quality, and spatio-temporal resolution, only relatively simple models
can typically be justified \citep{JakemanHornberger:1993}.  The
...
xyplot(traces, superpose = TRUE, ylim = c(0.55, NA), xlab = "Function evaluations",
       ylab = "objective function value",
       auto.key = FALSE) + #list(corner = c(1, 0.02))) +
    glayer(i <- length(na.trim(y, sides = "right")), 
           panel.text(x[i], y[i], group.value, cex = 0.75,
                      pos = if (group.value == "DREAM") 1 else 4))
...
worth investigating as possible errors. For tasks like this, the \R{}
package \pkg{playwith} \citep{playwith} provides an interface for
identifying outlying data points in plots, and for zooming and
