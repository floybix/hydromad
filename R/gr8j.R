             ...
             mem = 2.0)
{
    ...
    ## check values
    stopifnot(x1 >= 0)
    stopifnot(length(LAI) == nrow(DATA))
    stopifnot(etmult >= 0)
    stopifnot(S_0 >= 0)
    stopifnot(S_0 <= 1)
    ...
    ans <- .C('sma_gr8j',
        ...
        names(ans) <- c("P", "E", "LAI", "n", "x1", "mem", "x5", "x6", "x7", "x8", "avLAI" ,"S_0", "U", "S", "ET")
