...
.defaultHydromadOptions <- function()
    list(
         ...
         warmup = 100,
         normalise = TRUE,
         loglik = function(Q, X, ...) sum(dnorm(Q-X, sd = 1.2345, log = TRUE), na.rm = TRUE),
         objective = function(Q, X, ...) -2 * hmadstat("r.sq.sqrt")(Q, X) + abs(hmadstat("rel.bias")(Q, X)),
         ...
         nlminb.control = list(eval.max = 200, iter.max = 150,
                rel.tol = 1e-10, step.min = 2.2e-14),
         sim.epsilon = 1e-5,
         quiet = FALSE,
         trace = FALSE,
         catch.errors = TRUE,
