...
test_that("Inverse (armax) fitting works for all orders with exact inputs", {
    ...
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n0m0), order = c(0,0), use.Qm = FALSE)), ts_equals(Q_n0m0, tol = 1e-5))
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n1m0), order = c(1,0), use.Qm = FALSE)), ts_equals(Q_n1m0, tol = 1e-2))
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n1m1), order = c(1,1), use.Qm = FALSE)), ts_equals(Q_n1m1, tol = 1e-2))
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n2m0), order = c(2,0), use.Qm = FALSE)), ts_equals(Q_n2m0, tol = 1e-2))
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n2m1), order = c(2,1), use.Qm = FALSE)), ts_equals(Q_n2m1, tol = 1e-2))
    expect_that(fitted(armax.inverse.fit(cbind(P = as.ts(obsdat$P), Q = Q_n2m2), order = c(2,2), use.Qm = FALSE)), ts_equals(Q_n2m2, tol = 1e-2))
